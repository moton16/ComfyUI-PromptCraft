...
echo "========================================="
echo ""
echo "⚠️  以下 .py 文件的 docstring 版本号需手动更新（历史记录格式，sed 无法自动处理）："
echo "   - model_lora_loader.py  (文件头 '节点 vX.X.X')"
echo "   - llm_client.py         (docstring 末行 'VX.X.X —')"
echo "   - thinking_control.py   (docstring 末行 'VX.X.X —')"
echo "   - prompt_enhancer.py    (docstring 末行 'VX.X.X —')"
echo "   请在每文件末尾添加: ${FULL_UPPER} — <本版本改动描述>"
